...
/*
	...
	@@ Date Started : 22/05/2013
	@@ Date Finished : 13 /05/2013 
	@@ Filling couter isnt working right, given more time would work 100%.
	*/
	...
	// modue codes
	static String[] coursecode = new String[1000];
	static String[] courseTite = new String[1000];
	static int[] avaibleSpaces = new int[1000];
	static int[] limitSpace = new int[1000];
	...
	public static void main(String[] args) {
		...
		do{
			...
			System.out.println("ญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญญ");
			...
			for (i=0;i <lineArray.size();i++){
				if ( userName.equalsIgnoreCase(lineArray.get(i)) && password.equalsIgnoreCase(lineArray2.get(i))){
	...
	public static void 	editingStudentModules(String module, String title)
	{
		...
		saveStudents() ;
		editingModules(module,title);
	}


	public static void 	editingModules(String module, String title)
	{
		int where = findmodule(module);
		System.out.print(where);
		int tempSpaces = avaibleSpaces[where] + 1;
		if (where >= 0)
		{
			avaibleSpaces[where]= tempSpaces;
			saveModules();
		}else if (where == 0){
			System.out.print("Wasnt there bro");
		}
	}
	...
	private static void saveModules() {
		...
		try {
			...
			while (index < 1000 && studentID[index] != null) {
				StringBuilder builder = new StringBuilder();
				builder.append(coursecode [index]);
				builder.append(DELIMITER);
				builder.append(courseTite[index]);
				builder.append(DELIMITER);
				builder.append(avaibleSpaces[index]);
				builder.append(DELIMITER);
				builder.append(limitSpace[index]);
	...
	public static int findmodule(String name) {

		int counter = 0;
		while (counter < courseTite.length) {
			if (courseTite[counter] != null && courseTite[counter].equalsIgnoreCase(name)) {
				System.out.println("The Module was already in the Database");
				return counter; 
			}
			counter++;
		}		
	...
	private static void loadMoudles() {
		...
		try {
			...
			while ((record = bufferedReader.readLine()) != null) {
				...
				while (tokenizer.hasMoreElements()) {
					...
					coursecode[index] = (String)tokenizer.nextElement();
					courseTite[index] = (String)tokenizer.nextElement();
					avaibleSpaces[index] =  Integer.parseInt((String)tokenizer.nextElement());
					limitSpace[index] =  Integer.parseInt((String)tokenizer.nextElement());	
				}
				index++;
			}

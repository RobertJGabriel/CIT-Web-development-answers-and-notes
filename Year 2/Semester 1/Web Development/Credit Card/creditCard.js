...
function main() { //Start of Function , Which is to tell which Card it is .
	...
	if ((length <= 16 && length >= 13) && (numberOne == 4)) {
		alert("Visa"); //Is Visa Card
		math(creditCard,length);
	} else if ((length == 15) && (numberOne == 3)) {
		alert("American Express"); //Is Amercian Express
		math(creditCard,length);
	} else if ((length == 16) && (numberOne == 5)) {
		alert("MasterCard"); // Is Master Card
		math(creditCard,length);
	} else if ((length == 16) && (numberOne == 6)) {
		alert("Discover Card"); // Is Discover Card
		math(creditCard,length);
	} else {
		alert("I cant let you do that Dave :-O");
	}
} //End of Function 
...
function checksum(finished) { //Start of Function 
	if (finished%10 === 0) {
		alert('Credit Card is Valadate');
	} else {
		alert('Credit Card is enValad');
	}
} //End of Function